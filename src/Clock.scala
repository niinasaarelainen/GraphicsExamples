...
object StaticGraphics extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    val taustaAudio = new File("molli_KevytAmppari.wav");
    val taustaAudioStream = AudioSystem.getAudioInputStream(taustaAudio)
    val soundToPlay = AudioSystem.getClip()
    soundToPlay.open(taustaAudioStream)
    soundToPlay.loop(1000)
    ...
    val clock = new Panel {
      ...
      override def paintComponent(g: Graphics2D) = {
        ...
        g.setColor(new Color(180, 180, 235))
        ...
        MIDIPlayer.soita((Math.random()*20 +70).toInt, 0, 2)   // 2 = hyvätsonudit-Array:n 3. instrumentti
      }    
